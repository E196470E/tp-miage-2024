...
#AROWOLO Abdoul# (et #Bellegarde Angelau#)
...
Oui le code est structuré en couches, toutes les couches ne dépendent que d'une seule couche au maximum.
le principe de la boite noire est également appliqué, l'accès à la classe TodoItem par la classe TodoListController se fait
via le service TodoItemRepository.

Ce code peut poser des problèmes de testabilité, on aura des difficultés à isoler les tests unitaires
car faudra instancier des classes (TodoiItemRepository) pour tester des méthodes de TodoListController


*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

la RG se trouve ici : todolist-debut-ex1/src/main/java/com/acme/todolist/TodoListController.java
ligne 51 à ligne 55

*Cette règle est-elle facilement testable par un test unitaire ?*

Oui, on vérifie que le contenu de l'item a bien une note LATE lorsqu'on lui fournit un item ayant plus de 24H
...
docker run est la commande principale de Docker utilisée pour exécuter un conteneur à partir d'une image.

-it: permet à Docker de garder l'entrée standard ouverte même si aucun processus n'est attaché et alloue un terminal pseudo-TTY pour le conteneur.

-e POSTGRES_PASSWORD=password: Cela définit une variable d'environnement 'POSTGRES_PASSWORD' pour le conteneur PostgreSQL. La valeur 'password' de cette variable est le mot de passe de l'utilisateur par défaut (postgres) de PostgreSQL.

-p 5432:5432: Cela mappe le port 5432 du conteneur PostgreSQL au port 5432 de l'hôte. C'est le port par défaut utilisé par PostgreSQL pour les connexions client.

postgres: C'est le nom de l'image Docker à partir de laquelle le conteneur est créé. Dans ce cas, il s'agit de l'image officielle de PostgreSQL disponible sur le hub Docker.

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*

il est nul car on n'avait pas instancier de Todoitem
il faut faire une injection dans le controlleur
...
oui les données sont bien présentes
...
L'architecture hexagonale, également connue sous le nom de ports et adaptateurs, est un modèle d'architecture logicielle qui vise à rendre
les systèmes logiciels plus flexibles, modulaires et faciles à tester et en maintenant le cœur de l'application indépendant des détails d'implémentation externes.
Nous trouvons au centre de l'archirecture le domaine métier qui correspond à l'endroit où sera implémenter la logique métier. Les adaptateurs sont des composants
qui permettent au cœur de l'application d'interagir avec le monde extérieur. Ils sont responsables de la traduction des données et des actions entre le cœur de l'application et
les différents composants externes tels que les interfaces utilisateur, les bases de données, les services externes.  Les ports sont des interfaces qui définissent les points d'entrée et
de sortie du cœur de l'application. Ils représentent les actions que le cœur de l'application peut effectuer et les données qu'il peut recevoir ou envoyer.
